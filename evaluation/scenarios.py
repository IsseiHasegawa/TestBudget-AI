...
SCENARIOS: tuple[Scenario, ...] = (
    ...
    Scenario(
        name="luhn_accepts_anything",
        shape="direct",
        description="the card checksum is accidentally always true",
        path=f"{APP}/payment.py",
        old="    return total % 10 == 0",
        new="    return total % 10 >= 0",
    ),
    Scenario(
        name="email_tld_length",
        shape="direct, unrelated module",
        description="email validation demands a four letter top level domain",
        path=f"{APP}/profile.py",
        old=r'EMAIL_PATTERN = re.compile(r"^[^@\s]+@[^@\s]+\.[a-zA-Z]{2,}$")',
        new=r'EMAIL_PATTERN = re.compile(r"^[^@\s]+@[^@\s]+\.[a-zA-Z]{4,}$")',
    ),
    Scenario(
